            ...
            while (run == true && urlMessage != null)
            {
                ...
                try
                {
                    ...
                    if (htmlWeb.StatusCode == HttpStatusCode.OK)
                    {
                        ...
                        if (title.Contains("- CNN.com"))
                        {
                            title = title.Substring(0, title.Length - 10);
                        }
                        else if (title.Contains("- CNNPolitics.com") || title.Contains("| Bleacher Report"))
                        {
                            title = title.Substring(0, title.Length - 18);
                        }
                        else if (title.Contains("- CNN Video"))
                        {
                        ...
                        if (title.Length > 0)
                        {
                            string[] words = title.ToLower().Split(null);
                            ...
                            foreach (string w in words)
                            {
                                ...
                                if (word.Length > 0)
                                {
                                    InvertedItem newTitle = new InvertedItem(word, url, title, date);
                                    // Inserts new page to url table
                                    TableOperation insertNewTitle = TableOperation.InsertOrReplace(newTitle);
                                    titleTable.Execute(insertNewTitle);
                                }
                            }
                        }

                        PageItem newPage = new PageItem(url, title);
                        TableOperation insertNewPage = TableOperation.InsertOrReplace(newPage);
